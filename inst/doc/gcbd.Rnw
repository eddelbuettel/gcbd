...
\Abstract{
  ...
  the test platform, a few general lessons can be drawn. [ FIXME/TODO: EXPAND ]
}
...
can be dauting:  the \proglang{R} language and environment devotes a detailed
...
technical background is discussed. The implementation of our approach is
outlined in section 3. We provide our results in the section 4, before a
summary concludes.
...
function declaration, in this \code{sgemm} for single precision and
\code{dgemm} for double precision. The actual implementation becomes interchangeable
thanks to the API definition and can be supplied by different approaches
...
The third BLAS implementation is provided by the Goto BLAS. Upon the
...
along with implementations pf several BLAS function including \code{dgemm}, \code{dgemv},
and \code{dsymv} (and well as their single-precision counterparts).
...
Helper scripts will then verify this availability while the benchmark is executed.

\subsection{Benchmark Implementation}

The bechmarks described in this paper are produced by the package \pkg{gcbd}
which is part of the larger \pkg{gcb} package \citep*{rforge:gcb} on the
...
The benchmark execution can then be triggered by the script \code{benchmark.r} in the
...
that the GPU-based solution bears a clear penalty for smaller dimensions and
...
BLAS with Goto outperforming the Intel MKL. %[FIXME/REDO ?]
The Intel MKL is also seen as having an erratic performance. % which may call for redo.
...
solution dominates.  Interestingly, the Intel MKL also outperformns the Goto BLAS.
...
This outcome is similar to the earlier matrix crossproduct: the two multi-threaded BLAS
...
No LU decomposition with \pkg{gputools}. MKL performance is again similar to Goto.
