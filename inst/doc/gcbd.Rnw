...
\Abstract{
  ...
  performance of two GPU-based implementations for \proglang{R}
  \citep{RCore:R} provided by the packages \pkg{gputools}
  ...
  multiplication, QR decomposition, SVD decomposition and LU decomposition.
  The tests are performed from an end-user perspective, and `net' times
  (including all necessary data transfers) are compared.
...
I am very grateful for helpful and detailed comments and suggestions from
Roger Bivand, Allan Engelhardt, Bryan Lewis, Junji Nakano, Mark Seligmanm,
Brian Smith and Luke Tierney. All remaining errors are mine.
...
redistribution leading to fairly wide availability of Atlas.\footnote{The
  ...
  Atlas.}  Another popular BLAS implementation is Goto BLAS which is named
...
We used two different Atlas versions. The first one is the version in the
Ubuntu and Debian relases. It is based on Atlas 3.6.0 and built only for
single-threaded operation.  The second version is based on the current Atlas
development version 3.9.25 and built for multi-threaded mode.\footnote{We
  used a local binary package for this; a critical change enabling
  multi-threaded mode was also reported as Debian bug report \#595326 which
  will be reflected in future Debian packages.} We will refer to the second
variant as 'Atl39' when we report results below.
...
(single-precision) QR decompostion.  It clearly shows how the share of the
...
\texttt{gotoblas2-helper} package \citep{nakano_nakama:2009,Nakama:2010}. This package arranges
...
not possible the reconfigure the machines used here, it could be informative
...
something similar on OS X to measure the performance of the vecLib system.
...
section~\ref{subsec:hardware}) with times first shown on a regular scale (with
the matrix dimension on the horizontal axis and the elapsed time in seconds
on the vertical axis), and also in a so-called `log/log plot' with both
...
\begin{figure}[H]
  \centering
<<matmulti7,fig=TRUE,height=4.75,width=11>>=
D <- subset(i7[,-c(1:2,5)], type=='matmult')
op <- par(mfrow=c(1,2))
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob","gpu")], type='l', lty=1, lwd=2, col=cols,
        xlab="Matrix dimension", ylab="Time in seconds", main="Matrix Multiplication")
legend("topleft", legend=c("Ref","Atlas","Atl39","MKL","Goto","GPU"), bty="n", col=cols, lty=1, lwd=2)
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob","gpu")], type='l', lty=1, lwd=2, col=cols,
        xlab="Matrix dimension", ylab="Time in seconds (in logs)", main="Matrix Multiplication", log="xy")
legend("bottomright", legend=c("Ref","Atlas","Atl39","MKL","Goto","GPU"), bty="n", col=cols, lty=1, lwd=2)
par(op)
@
...
significantly. Similarlt, the single-threaded Atlas performance is also
...
single-threaded Atlas around $N=1000$ and approached the best performing
...
indistinguishable for this test.

\subsubsection{Matrix Multiplication: Xeon}

\begin{figure}[H]
  \centering
<<matmultxeon,fig=TRUE,height=4.75,width=11>>=
D <- subset(xeon[,-c(1:2,5)], type=='matmult')
op <- par(mfrow=c(1,2))
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob")], type='l', lty=1, lwd=2, col=cols,
        xlab="Matrix dimension", ylab="Time in seconds", main="Matrix Multiplication")
legend("topleft", legend=c("Ref","Atlas","Atl39","MKL","Goto"), bty="n", col=cols, lty=1, lwd=2)
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob")], type='l', lty=1, lwd=2, col=cols,
        xlab="Matrix dimension", ylab="Time in seconds (in logs)", main="Matrix Multiplication", log="xy")
legend("bottomright", legend=c("Ref","Atlas","Atl39","MKL","Goto"), bty="n", col=cols, lty=1, lwd=2)
par(op)
@
...
hand, on this platform the log/log plot offers a better differentitation
between the multithreaded BLAS implementations. Goto is clearly ahead of its
competitors. MKL shows slower slope and higher times for small sizes hinting
and some suboptimal configuration.  Atlas39 beats MKL for essentially all
...
\begin{figure}[H]
  \centering
<<qri7,fig=TRUE,height=4.75,width=11>>=
D <- subset(i7[,-c(1:2,5)], type=='qr')
op <- par(mfrow=c(1,2))
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob","gpu")], type='l', lty=1, lwd=2, col=cols,
        xlab="Matrix dimension", ylab="Time in seconds", main="QR Decomposition")
legend("topleft", legend=c("Ref","Atlas","Atl39","MKL","Goto","GPU"), bty="n", col=cols, lty=1, lwd=2)
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob","gpu")], type='l', lty=1, lwd=2, col=cols, log="xy",
...
As before, reference BLAS are behind all other implmentations. The
...
\begin{figure}[H]
  \centering
<<qrxeon,fig=TRUE,height=4.75,width=11>>=
...
\begin{figure}[H]
  \centering
<<svdi7,fig=TRUE,height=4.75,width=11>>=
...
\begin{figure}[H]
  \centering
<<svdxeon,fig=TRUE,height=4.75,width=11>>=
...
\begin{figure}[H]
  \centering
<<lui7,fig=TRUE,height=4.75,width=11>>=
D <- subset(i7[,-c(1:2,5)], type=='lu')
op <- par(mfrow=c(1,2))
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob")], type='l', lty=1, lwd=2, col=cols,
        xlab="Matrix dimension", ylab="Time in seconds", main="LU Decomposition")
legend("topleft", legend=c("Ref","Atlas","Atl39","MKL","Goto"), bty="n", col=cols, lty=1, lwd=2)
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","atl39","mkl","gotob")], type='l', lty=1, lwd=2, col=cols, log="xy",
...
slowest, single-threaded Atlas beats them clearly and loses equally clearly
to the multi-threaded BLAS. Multi-threaded Atlas is very close to Goto and
MKL, which are every so slightly ahead and essentially indistinguishable.
For this algorithm, no GPU-based performance numbers are available as the
current version of the \pkg{gputools} package does not provide a LU
decomposition.
...
\begin{figure}[H]
  \centering
<<luxeon,fig=TRUE,height=4.75,width=11>>=
...
implementation on the one hand and the single-threaded Atlas and the
...
exihibits some irregularity fir small-to-medium sized matrices.

\subsubsection{Comparison}

The charts shown above suggest a further analysis: a regression the logarithm
...
different slope estimates in Figure~\ref{fig:slopeloglog}.

\begin{figure}[H]
  \centering
<<loglogslopes,fig=TRUE,height=4.75,width=11>>=
LL <- loglogAnalysis()
DF <- LL[["slope"]]
...
These results re-confirm the rank-ordering seen in the individual charts
above:
\begin{itemize}
...
\item Single-threaded Atlas is clearly improving on the Reference
  BLAS but for most methods dominated by the other multi-threaded libraries and
  the GPU-based solution with the QR decompostion as an exception where both Atlas
...
\item GPU computing us see to have the lowest slope indicated the lowest cost
...
%        bty="n", col=paircols, lty=1, lwd=2)
% matplot(x=D[,"nobs"], y=D[,c("atlas","magmaAtlas","gotob","magmaGoto", "mkl", "magmaMkl")],
...
\begin{figure}[H]
  \centering
<<magmammqr,fig=TRUE,height=4.75,width=11>>=
op <- par(mfrow=c(1,2))
D <- subset(i7[,-c(1:2,5)], type=='matmult')
N <- cbind(D[,"nobs"],
...
invariant to the choice of multi-threaded BLAS as all three perform similarly
...
This has been reported to the \pkg{Magma} development team; hopefully a
future library version will allow these tests to be performed.

\begin{figure}[H]
  \centering
<<magmasvdlu,fig=TRUE,height=4.75,width=11>>=
op <- par(mfrow=c(1,2))
D <- subset(i7[,-c(1:2,5)], type=='svd')
N <- cbind(D[,"nobs"],
           D[,"atlas"]/D[,"magmaAtlas"],
...
use of just the accelerated BLAS.
