...
\Abstract{
  ...
  hybrid approach that is being implemented by the Magma project is also
  showing promise, as well as problems with stability in the current version.
}
...
and the elapsed time in seconds on the vertical axis), and also with elapsed
time on on a log-scale in order to better differentiate between smaller
values.
...
actually surpassing them on the chosen matrix sizes. On the log-chart, we can
also see a small edge for Goto over MKL.
...
legend("topleft", legend=c("Ref","Atlas","MKL","Goto","GPU"), bty="n", col=cols, lty=1)
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","mkl","gotob","gpu")], type='l', lty=1, lwd=2, col=cols, log="y",
...
legend("topleft", legend=c("Ref","Atlas","MKL","Goto"), bty="n", col=cols, lty=1)
matplot(x=D[,"nobs"], y=D[,c("ref","atlas","mkl","gotob")], type='l', lty=1, lwd=2, col=cols, log="y",
...
LU decomposition results are similar to earlier results. Reference BLAS are
by far the slowest, Atlas beats them clearly and looses equally clearly to
the multi-threaded BLAS.  MKL performance is once again very similar to Goto
for the LU decompostion.  For this algorithm, no GPU-based performance
numbers are available as the current version of the \pkg{gputools} package
does not provide a LU decomposition.
...
% matplot(x=D[,"nobs"], y=D[,c("atlas","magmaAtlas","gotob","magmaGoto", "mkl", "magmaMkl")],
%         type='l', lty=1, lwd=2, col=paircols, log="y",
...
increases performance becomes indistinguisable from a use of just the
accelerated BLAS.
...
BLAS improves on the reference BLAS but looses to multi-threaded BLAS.
...
field.  Yet the current implementation was seen to be not yet ready, as least
...
0.1-12 by \citet{Nakama:2010}. Apart from \pkg{gotoblas2-hel[er} and
