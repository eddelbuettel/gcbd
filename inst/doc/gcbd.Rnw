...
now.date <- strftime(Sys.Date(), "%B %m, %Y")
...
same name \citep{Tomov_et_al:2009,Tomov_et_al:2010}. We will capitalize the
...
stated goal of the Magma project it to \textsl{develop a dense linear algebra
  ...
  starting with current "Multicore+GPU" systems}. Such a flexible and hybrid
approach has the potential to improve upon solutions that use either the CPU
or the GPU. To the best of our knowledge, this paper offers the first
benchmark comparison of Magma versus BLAS implementations.
...
\item features tests via functions \code{hasMagma()} and \code{hasGputools()}
