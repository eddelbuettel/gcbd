...
if (!is.null(opt$help)) {
    cat(getopt(options, opt=argv, command="benchmarkMatrixMult.r", usage=TRUE))
    q(status=100)
}
...
cmd      <- paste('r -lgcbd       -e"cat(multBenchmark(', N, ",", n, '))"', sep="")
cmdmagma <- paste('r -lgcbd,magma -e"cat(multBenchmark(', N, ",", n, '))"', sep="")
cmdgpu   <- paste('r -lgcbd       -e"cat(multBenchmarkgputools(', N, ",", n, '))"', sep="")
...
res <- data.frame(type="matmult", nobs=N, nrun=n,
                  ref=ref,
